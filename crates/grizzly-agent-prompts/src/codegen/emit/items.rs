...
//! structs plus their `ToolDefinition` impl, and `Deserialize` params/enum
//! structs.
...
pub(super) fn emit_tool(
    ...
    code.blank();
    let params_ty = params_type_name(id, schema, resolved, crate_path);
    code.open(&format!("impl {crate_path}::ToolDefinition for {id} {{"));
...
/// The `ToolDefinition::Params` type for a tool: its synthesized struct for a
/// non-empty inline schema, the shared params file's own type for a
/// `params_from` reference, or `NoParams` for an empty inline schema.
fn params_type_name(
    id: &str,
    schema: &ToolSchemaRef,
    resolved: &ToolSchema,
    ...
    match schema {
        ToolSchemaRef::Inline(_) if resolved.params.is_empty() => format!("{crate_path}::NoParams"),
        ToolSchemaRef::Inline(_) => format!("{id}Params"),
        ToolSchemaRef::Shared(target) => target.clone(),
    }
}
