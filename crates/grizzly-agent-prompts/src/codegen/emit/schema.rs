...
//! `spec()`; here we only emit the macro invocation, referencing the
//! `serde_json` re-export through the configured crate path. Parameter order
//! follows the source `tool_schema`: `object` with typed, described
//! `properties`, a `required` array (omitted when empty), and
...
/// assembled schema for `schema`, built via `<crate_path>::serde_json`.
pub(super) fn emit_schema(code: &mut Code, crate_path: &str, schema: &ToolSchema) {
    code.open(&format!("parameters: {crate_path}::serde_json::json!({{"));
