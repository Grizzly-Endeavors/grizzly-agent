...
use super::parse::{
    RawAnnotations, RawFile, RawVariable, TokenizeError, parse_file, parse_raw_param, tokenize_body,
};
...
fn build_tool_schema(raw: &RawFile, map: &Hash) -> Result<ToolSchema, PromptError> {
    ...
    for (key, value) in map {
        ...
        let raw_param = parse_raw_param(&raw.rel_path, value)?;
...
fn validate_param(
    raw: &RawFile,
    name: &str,
    raw_param: &super::parse::RawParam,
) -> Result<Param, PromptError> {
    if !is_valid_placeholder_name(name) {
        return Err(PromptError::PlaceholderName {
            path: raw.rel_path.clone(),
            name: name.to_owned(),
...
fn build_variables(
    raw: &RawFile,
    placeholders: &[String],
    map: &BTreeMap<String, RawVariable>,
) -> Result<Vec<Variable>, PromptError> {
    let mut out = Vec::new();
    for name in placeholders {
        let entry = map
