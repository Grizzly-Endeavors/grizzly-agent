...
- **Generated tools implement `ToolDefinition`**, binding each tool's spec to its params type (inline, shared via `params_from`, or `NoParams`). `spec()` is reachable only through that impl (ADR-0012); the tool unit struct, its inherent `NAME` constant, params structs, enums, and prompt renderers are otherwise unchanged. A caller needs `ToolDefinition` in scope to call `Tool::spec()`.
